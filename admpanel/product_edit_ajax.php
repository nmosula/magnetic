...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if($image_res){
		...
		if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
		{
			...
			$res = $db->query("select image from ".$table_product." where id='".$edit."'");
			$row = $db->fetch_array($res);
			
			$image= $row["image"];
			$image_small	= "sm_".$image;
			$path=$destination_folder.$image;
			if (file_exists($path)) @unlink($path);
			$path2=$destination_folder."small/".$image_small;
			if (file_exists($path2)) @unlink($path2);
		
			
			$query ="update ".$table_product." set image='".$new_file_name."' where id='$edit'";
			$res=$db->query ($query);
			...
			<p class='text-success'>Фото успішно додане!</p>
			</div>";
